...
function render(){
  ...
  $("page-calendar").innerHTML=`
    ...
    <div class="card">
      <div class="cal-head"><button class="iconbtn ghost" onclick="A.calNav(-1)">${ICON.back}</button><div class="mo">${cur.toLocaleDateString(undefined,{month:"long",year:"numeric"})}</div><button class="iconbtn ghost" onclick="A.calNav(1)">${ICON.chevron}</button></div>
    ...
    <div class="card">
      ...
      ${dayTasks.map(t=>{const on=taskDoneOn(t,S.selDate);return `<div class="row"><button class="chk ${on?"on":""}" onclick="toggleTaskOn('${t.id}','${S.selDate}')">${ICON.check}</button><div class="grow"><div class="title ${on?"done-text":""}">${esc(t.text)}</div><div class="kind">${t.area==="together"?"Together":"For "+esc(profOf(t.area).name)}${repeatLabel(t)?" · "+repeatLabel(t):""}</div></div><button class="editp" onclick="A.openTaskModal(null,null,'${t.id}')">${ICON.edit}</button></div>`;}).join("")||'<div class="empty">No tasks for this day.</div>'}
    </div>`;
}
