...
export default handle(async (req, headers) => {
  ...
  if (!entitled(house)) {
    return json(200, { say: "This household's subscription has lapsed, so I'm resting. The rule of life, the calendar, and the prayers all still work. Renew from Settings when you're ready.", actions: [], entitled: false }, headers);
  }
  ...
  if (used >= cap) {
    return json(200, { say: `We've used today's ${cap} messages, so I'll rest until tomorrow. The house keeps its rule without me: the bells still ring and everything you've set stays set.`, actions: [], quota: { used, cap } }, headers);
  }
  ...
  if (error) return json(200, { say: "[" + error + "]", actions: [], quota: { used, cap } }, headers);
  if (!parsed || typeof parsed !== "object") {
    ...
    if (second.error || !parsed || typeof parsed !== "object") {
      return json(200, { say: "I hit a snag — try that again.", actions: [], quota: { used, cap } }, headers);
    }
  }
  // Count it only once Claude has actually answered.
